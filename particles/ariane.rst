...
* Column 3: Spatial grid index (Z)
	...
	* Since Ariane starts counting from 1, a "1" or "-1" here means the first depth grid box. The NEMO output grid boxes are 1 metre deep for the first few metres. This means that the second particle in this example (whose Z index is -1.5 will be identical to a particle at depth -1 if they share the same X, Y, and T values.
